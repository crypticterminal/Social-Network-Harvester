...
def update_user_status_from_batch(harvester, snhuser, fbstatus_page):
    ...
    if status_count:
        usage = resource.getrusage(resource.RUSAGE_SELF)
        logger.debug(u"Adding %d new statuses: %s Mem:%s KB" % (status_count, harvester, unicode(getattr(usage, "ru_maxrss")/(1024.0))))
...
def update_user_comments_from_batch(harvester, status, fbcomments_page):
    ...
    if comment_count:
        usage = resource.getrusage(resource.RUSAGE_SELF)
        logger.debug(u"Adding %d new comments: %s Mem:%s KB" % (comment_count, harvester, unicode(getattr(usage, "ru_maxrss")/(1024.0))))
